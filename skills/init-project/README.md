...
| Progressive disclosure setup | docs/knowledge/ + .claude/rules/ | "Any knowledge files need a new rule file? Any inline refs to add?" |
...
- `docs/experiment-registry/registry/` — YAML-based experiment registry, managed by the `exp-registry` CLI (`pip install exp-registry`)
...
Pre-commit quality check. Identifies changed `.py` files, runs `ruff check --fix` + `ruff format`, then `pytest`. Reports results in a structured table — does **not** auto-fix test failures. Runs with `permissionMode: bypassPermissions` and `maxTurns: 15` for unattended execution.

### planner (opus)

Read-only codebase researcher for `/brainstorming` and `/writing-plans` workflows. Systematically searches code, outputs structured findings (relevant files, existing patterns, suggestions, risks). Runs with `permissionMode: plan` (no writes).
