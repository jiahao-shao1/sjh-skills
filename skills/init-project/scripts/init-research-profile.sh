...
| Agent | Purpose | Model | Permission | Trigger |
|-------|---------|-------|------------|---------|
| `planner` | Codebase research | opus | plan | brainstorming/writing-plans phases |
| `code-verifier` | ruff + pytest | haiku | bypassPermissions | After code changes, before commit |
| `domain-expert` | Domain expertise | opus | plan | When touching core domain code |

### Experiment Registry

Experiments are tracked via `exp-registry` CLI (`pip install exp-registry`). YAML files in `docs/experiment-registry/registry/`.
